...
from model.TwoStepBaseline import TwoStepRF, TwoStepRidge, TwoStepTarNet, TwoStepRF_modi, TwoStepRidge_modi, TwoStepTarNet_modi, RF
...
a_file = open("data/sim_dict_actg.pkl", "rb")
sim_dict = pickle.load(a_file)

d = sim_dict['x_unc'].shape[1]
n_unc_size = [1*d, 2*d, 3*d, 4*d, 6*d, 8*d]
...
for i in range(n_run):
    ...
    x_c, t_c, y_c = x_conf, t_conf, y_conf
    ...
    for j_ind, j in enumerate(n_unc_size):
        print('###################     Training CorNet on ', j,' samples.     ####################') 
        x_uc, y_uc, t_uc = torch.Tensor(x_unc[:j,].copy()), torch.Tensor(y_unc[:j,].copy()), torch.Tensor(t_unc[:j,].copy())
        #Validation UNC set
        x_u_t, x_val, t_u_t, t_val, y_u_t, y_val = train_test_split(x_uc, t_uc, y_uc, test_size = 0.2)

        #Estimate propoensity score
        clf = LogisticRegression(random_state=0).fit(x_unc[:j,], t_unc[:j,].flatten())
        prop_score = clf.predict_proba(x_unc[:j,])[:,1].reshape(-1,1)
        ...
        lambd_ada_l = [np.sqrt((1/d_hidden)*np.log(d_hidden)/j)]
        alpha_l = [1.]
        ...
        #Train 2StepRidge
        baseline_ridge = TwoStepRidge(t_unc[:j,], t_c, x_unc[:j,], x_c, y_unc[:j,], y_c, x_test, q)
        #Train 2StepRF
        baseline_rf = TwoStepRF(t_unc[:j,], t_c, x_unc[:j,], x_c, y_unc[:j,], y_c, x_test, q)
        ...
        #Train 2StepRidge
        baseline_ridge = TwoStepRidge(t_unc[:j,], t_conf, x_unc[:j,], x_conf, y_unc[:j,], y_conf, x_test, q)
        cate_base_ridge.append(baseline_ridge[0])
        #Train 2StepRF
        baseline_rf = TwoStepRF(t_unc[:j,], t_conf, x_unc[:j,], x_conf, y_unc[:j,], y_conf, x_test, q)
        cate_base_rf.append(baseline_rf[0])

        #Train 2StepRidge - prop
        baseline_ridge_prop = TwoStepRidge(t_unc[:j,], t_conf, x_unc[:j,], x_conf, y_unc[:j,], y_conf, x_test, q=prop_score)
        cate_base_ridge_prop.append(baseline_ridge_prop[0])
        #Train 2StepRF - prop
        baseline_rf_prop = TwoStepRF(t_unc[:j,], t_conf, x_unc[:j,], x_conf, y_unc[:j,], y_conf, x_test, q=prop_score)
        cate_base_rf_prop.append(baseline_rf_prop[0])

        #Train 2StepRidge - modi
        baseline_ridge_modi = TwoStepRidge_modi(t_unc[:j,], t_conf, x_unc[:j,], x_conf, y_unc[:j,], y_conf, x_test)
        cate_base_ridge_modi.append(baseline_ridge_modi[0])
        #Train 2StepRF - modi
        baseline_rf_modi = TwoStepRF_modi(t_unc[:j,], t_conf, x_unc[:j,], x_conf, y_unc[:j,], y_conf, x_test)
        ...
        for lambd_ada in lambd_ada_l:
            for alpha in alpha_l:
                for lambd_h_c in lambd_h_c_l:
                        ...
                        if (((y_v - (t_v*o1+(1-t_v)*o0))**2).mean()).detach().numpy() < best_loss:
                        #if pseudo_pehe(x_v, t_v, y_v, tn_reg.predict_naive(x_v)).detach().numpy() < best_loss:
                            ...
                            #best_loss = pseudo_pehe(x_v, t_v, y_v, tn_reg.predict_naive(x_v)).detach().numpy()
        ...
        best_params_tn_reg['lambd_delta'] = np.sqrt((1/d_hidden)*np.log(d_hidden)/j)#best_params_tn_reg['lambda_h_c']*np.sqrt(n_conf/n_unc)##np.sqrt(np.log(d_hidden)/n_unc)#np.sqrt(2*np.log(d_hidden)/n_unc)
        ...
        for lambd_h_c in lambd_h_c_l:
            ...
            rep, o1, o0 = tn(x_v)
            if (((y_v - (t_v*o1+(1-t_v)*o0))**2).mean()).detach().numpy() < best_loss:
            #if pseudo_pehe(x_v, t_v, y_v, tn.predict_naive(x_v)).detach().numpy() < best_loss:
                best_tn = deepcopy(tn)
                best_params_tn['lambda_h_c'] = lambd_h_c    
                #best_loss = pseudo_pehe(x_v, t_v, y_v, tn.predict_naive(x_v)).detach().numpy()
                best_loss = (((y_v - (t_v*o1+(1-t_v)*o0))**2).mean()).detach().numpy()
        ...
        best_params_tn['lambd_delta'] = np.sqrt((1/d_hidden)*np.log(d_hidden)/j)#best_params_tn['lambda_h_c']*np.sqrt(n_conf/n_unc)#np.sqrt(0.1*np.log(d_hidden)/n_unc)##np.sqrt(np.log(d_hidden)/n_unc)#np.sqrt(2*np.log(d_hidden)/n_unc)
        ...
        #Train head regulaized on uncofounded data (Step 2)
        best_params_mtl['lambd_delta'] = np.sqrt((1/n_hidden)*np.log(d_hidden)/j)#best_params_mtl['lambda_h_c']*np.sqrt(n_conf/n_unc)#np.sqrt(0.1*np.log(d_hidden)/n_unc)#best_params_tn_reg['lambda_h_c']*np.sqrt(n_conf/n_unc)#np.sqrt(np.log(d_hidden)/n_unc)#np.sqrt(2*np.log(d_hidden)/n_unc)
        ...
        #Train head regulaized on uncofounded data (Step 2)
        best_params_mtl_bal['lambd_delta'] = np.sqrt((1/d_hidden)*np.log(d_hidden)/j)#best_params_mtl_bal['lambda_h_c']*np.sqrt(n_conf/n_unc)#np.sqrt(0.1*np.log(d_hidden)/n_unc)#np.sqrt(np.log(d_hidden)/n_unc)#np.sqrt(2*np.log(d_hidden)/n_unc)
        ...
        #Train 2StepNN
        baseline_tn = TwoStepTarNet(best_tn, t_unc[:j,], x_unc[:j,], y_unc[:j,], x_test, q)
        cate_base_nn.append(baseline_tn)
    
        #Train 2StepNN - prop
        baseline_tn_prop = TwoStepTarNet(best_tn, t_unc[:j,], x_unc[:j,], y_unc[:j,], x_test, q=prop_score)
        cate_base_nn_prop.append(baseline_tn_prop)
    
        #Train 2StepNN - modi
        baseline_tn_modi = TwoStepTarNet_modi(best_tn, t_unc[:j,], x_unc[:j,], y_unc[:j,], x_test)
        ...
        for lambd_avg in lambd_avg_l:
            ...
            #if pseudo_pehe(x_v, t_v, y_v, tn_conf.predict_naive(x_v)).detach().numpy() < best_loss:
                best_tn_avg_u = deepcopy(tn_unc) 
                best_params_avg['lambda_avg'] = lambd_avg    
                #best_loss = pseudo_pehe(x_v, t_v, y_v, tn_conf.predict_naive(x_v)).detach().numpy()
...
# #Save data
# setting = 'ACTG'
# folder = 'Experiments/Real_World/n_unc/'+ setting +'/'
# 
# np.save(folder+'exp_'+setting+'_trans_reg_l', cate_trans_reg_l)
# np.save(folder+'exp_'+setting+'_trans_l', cate_trans_l)
# np.save(folder+'exp_'+setting+'_conf_l', cate_conf_l)
# np.save(folder+'exp_'+setting+'_mtl_l', cate_mtl_l)
# np.save(folder+'exp_'+setting+'_mtl_delta_1_l', cate_mtl_delta_1_l)
# np.save(folder+'exp_'+setting+'_mtl_bal_l', cate_mtl_bal_l)
# np.save(folder+'exp_'+setting+'_mtl_bal_delta_1_l', cate_mtl_bal_delta_1_l)
# np.save(folder+'exp_'+setting+'_trans_delta_1_l', cate_trans_delta_1_l)
# np.save(folder+'exp_'+setting+'_trans_delta_2_l', cate_trans_delta_2_l)
# np.save(folder+'exp_'+setting+'_trans_out_l', cate_trans_out_l)
# np.save(folder+'exp_'+setting+'_trans_out_1_l', cate_trans_out_1_l)
# np.save(folder+'exp_'+setting+'_trans_out_2_l', cate_trans_out_2_l)
# np.save(folder+'exp_'+setting+'_trans_bal_l', cate_trans_bal_l)
# np.save(folder+'exp_'+setting+'_trans_bal_out_1_l', cate_trans_bal_out_1_l)
# np.save(folder+'exp_'+setting+'_trans_bal_out_2_l', cate_trans_bal_out_2_l)
# np.save(folder+'exp_'+setting+'_unc_l', cate_unc_l) 
# np.save(folder+'exp_'+setting+'_avg_l', cate_avg_l) 
# np.save(folder+'exp_'+setting+'_weight_l', cate_weight_l) 
# np.save(folder+'exp_'+setting+'_base_rf_l', cate_base_rf_l) 
# np.save(folder+'exp_'+setting+'_base_ridge_l', cate_base_ridge_l)
# np.save(folder+'exp_'+setting+'_base_nn_l', cate_base_nn_l)
# np.save(folder+'exp_'+setting+'_base_rf_prop_l', cate_base_rf_prop_l) 
# np.save(folder+'exp_'+setting+'_base_ridge_prop_l', cate_base_ridge_prop_l)
# np.save(folder+'exp_'+setting+'_base_nn_prop_l', cate_base_nn_prop_l)
# np.save(folder+'exp_'+setting+'_base_rf_modi_l', cate_base_rf_modi_l) 
# np.save(folder+'exp_'+setting+'_base_ridge_modi_l', cate_base_ridge_modi_l)
# np.save(folder+'exp_'+setting+'_base_nn_modi_l', cate_base_nn_modi_l)
# np.save(folder+'exp_'+setting+'_cate_test', cate_test)
# =============================================================================
...
# setting = 'ACTG'
# folder = 'Experiments/Real_World/n_unc/'+ setting +'/'
# cate_trans_reg = np.load(folder+'exp_'+setting+'_trans_reg_l.npy')
# cate_trans = np.load(folder+'exp_'+setting+'_trans_l.npy')
# cate_conf = np.load(folder+'exp_'+setting+'_conf_l.npy')
# cate_mtl = np.load(folder+'exp_'+setting+'_mtl_l.npy')
# cate_mtl_delta_1 = np.load(folder+'exp_'+setting+'_mtl_delta_1_l.npy')
# cate_mtl_bal = np.load(folder+'exp_'+setting+'_mtl_bal_l.npy')
# cate_mtl_bal_delta_1 = np.load(folder+'exp_'+setting+'_mtl_bal_delta_1_l.npy')
# cate_trans_delta_1 = np.load(folder+'exp_'+setting+'_trans_delta_1_l.npy')
# cate_trans_delta_2 = np.load(folder+'exp_'+setting+'_trans_delta_2_l.npy')
# cate_trans_out = np.load(folder+'exp_'+setting+'_trans_out_l.npy')
# cate_trans_out_1 = np.load(folder+'exp_'+setting+'_trans_out_1_l.npy')
# cate_trans_out_2 = np.load(folder+'exp_'+setting+'_trans_out_2_l.npy')
# cate_trans_bal = np.load(folder+'exp_'+setting+'_trans_bal_l.npy')
# cate_trans_bal_out_1 = np.load(folder+'exp_'+setting+'_trans_bal_out_1_l.npy')
# cate_trans_bal_out_2 = np.load(folder+'exp_'+setting+'_trans_bal_out_2_l.npy')
# cate_unc = np.load(folder+'exp_'+setting+'_unc_l.npy') 
# cate_avg = np.load(folder+'exp_'+setting+'_avg_l.npy') 
# cate_weight = np.load(folder+'exp_'+setting+'_weight_l.npy') 
# cate_base_rf_l = np.load(folder+'exp_'+setting+'_base_rf_l.npy') 
# cate_base_ridge_l = np.load(folder+'exp_'+setting+'_base_ridge_l.npy')
# cate_base_nn_l = np.load(folder+'exp_'+setting+'_base_nn_l.npy')
# cate_base_rf_prop_l = np.load(folder+'exp_'+setting+'_base_rf_prop_l.npy') 
# cate_base_ridge_prop_l = np.load(folder+'exp_'+setting+'_base_ridge_prop_l.npy')
# cate_base_nn_prop_l = np.load(folder+'exp_'+setting+'_base_nn_prop_l.npy')
# cate_base_rf_modi_l = np.load(folder+'exp_'+setting+'_base_rf_modi_l.npy') 
# cate_base_ridge_modi_l = np.load(folder+'exp_'+setting+'_base_ridge_modi_l.npy')
# cate_base_nn_modi_l = np.load(folder+'exp_'+setting+'_base_nn_modi_l.npy')
# cate_test = np.load(folder+'exp_'+setting+'_cate_test.npy', allow_pickle = True)
# 
# 
# =============================================================================
